...
class Welcome extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->input->post('submit')){
			...
			if($this->session->userdata('type') == 'Student')
			{
				...
				print_r($date);
				$check_in_date = substr('Vikram sarabhai2018-12-09',strlen('Vikram Sarabhai'),10);
				print_r($check_in_date);*/

				//$var = $this->public_model->total_rooms(1,'AC');
				/*$var = 
				print_r ($var);
				exit();*/

				$total_rooms = $this->public_model->total_rooms(1,'AC');
				$booked_rooms = $this->public_model->booked_ac(1,'2018-06-26','2018-06-28');
				...
				/*$var = $this->public_model->guesthousenum('J.C. Bose');
				echo($var);
				$var = $this->public_model->room_status_ac($var);
				print_r($var);*/
				/*$var = $this->public_model->page(2,'faculty_incharge','booking','Official');
				print_r($var);
				exit();*/

				redirect('home');
			}
			else if($this->session->userdata('type') == 'Caretaker')
			{
				redirect('caretaker');
			}
			...
			else if($this->session->userdata('type') == 'Dean')
			{
				redirect('dean');
			}
			else if($this->session->userdata('type') == 'Faculty Incharge')
			{
		...
		if($this->input->post('submit'))
		{
			if($st1 < $st2){
				...
				if($this->session->userdata('type') == 'Dean'){
					redirect('test4/'.$t1.'/'.$t2.'/'.$this->input->post('rooms'));
				}				

				$status1 = $this->public_model->check_availability($t1,'AC');
				$booked_ac = $this->public_model->booked_ac($t1);
				$status1 = $status1 - $booked_ac;

				$status2 = $this->public_model->check_availability($t2,'Non-AC');
				$booked_nac = $this->public_model->booked_nac($t1);
				$status2 = $status2 - $booked_nac;
	...
	public function test4($t1,$t2,$rooms)
	{
		$data = $this->general();
		...
		$status2 = $this->public_model->check_availability($t1,'Non-AC');
		$booked_nac = $this->public_model->booked_nac($t1);
		$status2 = $status2 - $booked_nac;
		...
		$data['pagename'] = 'Dean Reject';
		$data['pagetitle'] = 'Dean Reject';
		$data['metadesc'] = '';
		$data['metakey'] = '';
	...
	public function application_form()
	{
		...
		if($this->input->post('submit')){
            ...
            if(!empty($_FILES['document']['name'])){
           	...
           	$this->public_model->insert($document);
	...
	public function room_status_ac_dean()
	{
		$var = $this->input->post('name');
		$guesthousenumber = substr($var,0,1);
		$check_in_date = substr($var,1,10);
		$check_out_date = substr($var,1 + strlen($check_in_date),10);

		//$guesthousenum = $this->public_model->guesthousenum($guesthousename);
		//$count = $this->public_model->room_status_dean($check_in_date,$guesthousenumber,'AC');
	...
	public function room_status_non_ac_dean()
	{
		$var = $this->input->post('name');
		$guesthousenumber = substr($var,0,1);
		$check_in_date = substr($var,1,10);
		$check_out_date = substr($var,1 + strlen($check_in_date),10);
		//$guesthousenum = $this->public_model->guesthousenum($guesthousename);
		//$count = $this->public_model->room_status_dean($check_in_date,$guesthousenumber,'Non-AC');
		...
		if($this->session->userdata('userid') != '' && $this->session->userdata('type') == 'Dean')
		{
			...
			if($this->input->post('approve'))
			{
				if(!empty($_POST['guesthouse'])){
					$this->public_model->status('approve','dean',$application_number);
	...
	public function test2($application_number)
	{
		if($this->session->userdata('userid') != '' && $this->session->userdata('type') == 'Faculty Incharge')
		{
			...
			$data['room_number'] = $this->public_model->room_number('$chkin');
			if($this->input->post('submit')){
				if(!empty($_POST['room'])){
